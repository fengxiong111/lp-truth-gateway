...
默认分支的 `[LP_TRUTH] <EVM 地址>` Issue Queue 使用 DexScreener 候选发现和 GeckoTerminal OHLCV（5m/30m/1h/1d 请求），并记录 freshness、price conflict、wick penalty。Uniswap/RPC、OKX、Revert、VFAT 未配置授权时不伪造数据。
