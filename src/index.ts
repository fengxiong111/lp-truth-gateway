...
async function verifyPools(candidates: PoolCandidate[], preferredPool?: string | null): Promise<{ candidates:PoolCandidate[]; verifiedPools:OnchainPoolTruth[]; selected:PoolCandidate|null; onchain:OnchainPoolTruth|null; statuses:SourceStatus[] }> {
  ...
  const preferred = preferredPool
    ? enriched.find((candidate)=>candidate.poolAddress.toLowerCase()===preferredPool.toLowerCase()) ?? null
    : null;
  ...
  const selected=preferred??ranked[0]??null;
...
export async function buildTruth(address: string, options: BuildTruthOptions = {}): Promise<TruthArtifact> {
  ...
  const verified=await verifyPools(discovered, options.preferredPool);
